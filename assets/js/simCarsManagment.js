//import modules
import { simCard } from "./tools/card.js";
...
window.onload = () => {
  ...
  fetch(" ../routers/simCards/get_simCards.php").then((res) =>
    res.json().then((data) => {
      data.map((ele) => {
        let type = ""
        if (ele.type == 0) type = "Vodafone"
        else if (ele.type == 1) type = "Orange"
        else if (ele.type == 2) type = "Etisalat"
        else if (ele.type == 3) type = "We"
        tableBody.innerHTML += simCard(ele.number, ele.name, ele.id,type);
      });
...
async function autoSelectType() {
  ...
  item.addEventListener("input", (e) => {
    if (!accept) {
      if (!e.target.value.startsWith("010") || !e.target.value.startsWith("012") || !e.target.value.startsWith("011") || !e.target.value.startsWith("015")) {
        type.value = ""
        type.dataset.type=""
      }
